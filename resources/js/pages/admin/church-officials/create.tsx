...
interface ChurchOfficialCreateProps {
  communities: Community[];
  positions: Position[];
}

export default function ChurchOfficialCreate({ communities, positions }: ChurchOfficialCreateProps) {
  ...
  const submit: FormEventHandler = (e) => {
    e.preventDefault();
    post(route('admin.church-officials.store'), {
      forceFormData: true,
      preserveScroll: true,
    });
  };
  ...
  return (
    <AuthenticatedLayout>
      <Head title="Tambah Pengurus Gereja" />
      ...
      <div className="dashboard-gradient min-h-screen" style={{ background: 'linear-gradient(135deg, #0f2027, #203a43, #2c5364)' }}>
        <div className="container mx-auto px-4 py-8">
          ...
          {/* Header */}
          <div className="bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 backdrop-blur-sm rounded-2xl shadow-2xl p-8 mb-8 border border-white/20">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-4xl font-bold text-white mb-2 flex items-center gap-3">
                  <Users className="h-10 w-10 text-white" />
                  Tambah Pengurus Gereja
                </h1>
                <p className="text-blue-100 text-lg">
                  Tambah data pastor paroki atau pengurus lingkungan
                </p>
              </div>
